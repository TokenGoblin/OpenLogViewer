...
| A MaxxECU on USB is in no list of ports | It never becomes a COM port | This is normal and not a fault. Its USB is an FTDI device, so it has its own entry in **Connect ▾** reading `MaxxECU (…) (USB)`. See [MaxxECU](live-connection.md#maxxecu) |
