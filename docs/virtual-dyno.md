...
**What you type is remembered**, in [`vehicle.json`](configuration.md#vehiclejson).
...
- [Estimating power](user-guide.md) — the simpler single-figure estimate under **Tools ▸ Estimate power…**
- [Configuration](configuration.md#vehiclejson) — where the car you type is stored
- [Command line](command-line.md#capturing-parts-of-the-interface) — `--dyno`, which saves the window to a PNG
- [VE calibration](ve-calibration.md) — the other analysis that reads mixture against target
