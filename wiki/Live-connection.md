...
| Any OBD2 vehicle | ELM327 adapter: USB, Bluetooth LE or Wi-Fi | No — see [OBD2](OBD2) |
| Subaru, over SSM | Serial / USB | No, but you supply an address list — see [Subaru SSM](Subaru-SSM) |

## Definition files

### What they are and why they are needed

A **definition file** — a TunerStudio `.ini` — describes exactly what an ECU's
data means.
...
1. `%USERPROFILE%\OpenLogViewer\ECU definitions` — and its sub-folders
2. `%USERPROFILE%\.efiAnalytics\TunerStudio\config\ecuDef`
3. `Documents\TunerStudioProjects` — each project folder
...
**Tools ▸ ECU definition files…** opens the folder and creates it if it does not
...
Nothing is downloaded. OpenLogViewer never uses the internet for this — the
...
1. **Tools ▸ Connect**, or **Connect ▾** in the toolbar.
...
- **Connect as an OBD2 adapter** — for a generic cable
- **Connect to a Wi-Fi OBD2 adapter** — see [OBD2](OBD2)
- **Connect over SSM (Subaru)** — see [Subaru SSM](Subaru-SSM)
...
**To verify:** switch to **Gauges** and confirm the readings move as expected —
...
port numbers out and reuses them — the same Speeduino is COM7 today and COM12
...
| **Ctrl+K** | The same |
| **Connect ▾** | Devices that have answered before are grouped above the rest, most recently used first |
...
| --- | --- | --- |
| Toolbar **● Record…** / **■ Stop recording** | — | Starts and stops a recording. You choose the moment, the name and the folder |
| **Tools ▸ Record as soon as I connect** | **Off** | When on, connecting starts a recording immediately |
| **Tools ▸ Open the recordings folder** | — | Opens `%USERPROFILE%\OpenLogViewer\Logs` |
...
pan** — from then on you are reading history. **View ▸ Reset zoom** goes back to
watching.
...
where the wideband is the slow part. The faster rates are for transients — accel
enrichment, knock, per-cylinder events — and cost what they sound like they cost:
...
**Tools ▸ Read the block in one request.** Default **off**.
...
Switch to **Gauges** in the toolbar, or **View ▸ Gauges**.
...
They are kept rather than dropped — the channel is worth seeing even when the
firmware has not said what a normal value looks like — and show as a reading
without a scale.

OBD2 gauges are drawn differently; see [OBD2 ▸ Gauges](OBD2#gauges).
...
— the same things TunerStudio reads continuously. **Nothing is written unless you
...
| --- | --- | --- |
| The device is not in the **Connect ▾** list | It is a Bluetooth LE adapter that has not paired, or a Wi-Fi dongle | BLE adapters appear with `(Bluetooth LE)`; Wi-Fi dongles appear in no list — see [OBD2](OBD2) |
...
| Readings look plausible but are wrong | The wrong definition matched | Compare a known value — battery voltage, coolant — against a gauge you trust |
...
- [OBD2](OBD2) — standard vehicles through an ELM327
