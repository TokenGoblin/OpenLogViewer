...
| Change it | **File â–¸ Data folder â–¸ Change the folderâ€¦** |
| Open it | **File â–¸ Data folder â–¸ Open the folder** |
...
of them *while it is still being written* â€” a long session is tens of megabytes of
...
If the chosen folder cannot be written â€” an unplugged drive, a network share that
is not there â€” the application falls back to the default location rather than
...
| `themeId` | string | `midnight` | Any theme id â€” the lower-case hyphenated scheme name | Active colour scheme |
...
| `units` | string | `AsReported` | `AsReported`, `Metric`, `Imperial` | Which units readings are displayed in. **Case-sensitive** â€” an unrecognised value falls back to `AsReported` |
| `knownEcus` | object | *(unset)* | hardware id â†’ name | What answered on each serial device |
| `ecuLastUsed` | object | *(unset)* | hardware id â†’ ISO-8601 timestamp | When each device was last connected to |
| `obd2BatchDeaths` | object | *(unset)* | adapter id â†’ count | How many times a batched OBD2 request killed the link on that adapter |
...
are for transients, and cost disk â€” at 100 Hz a rusEFI's 823 channels are about
...
does not. See [Live
connection](Live-connection#reading-the-block-in-one-request).
...
the ECU â€” a Speeduino shows up as "Arduino Mega 2560" â€” which is why what answered
is remembered separately. **Tools â–¸ Forget remembered ECUs** clears
...
different adapter starts clean. See [OBD2 â–¸ Adapters that cannot survive being
asked](OBD2#adapters-that-cannot-survive-being-asked).
...
one log applies to any other log â€” or live session â€” carrying those names.
...
[Calculated channel](Calculated-channels) definitions, held by name and
expression. One that does not fit the open log is reported in the sidebar rather
than dropped.

Managed from **Æ’ Add calculated channel**.
...
| Smoothing | `None`, `Light`, `Medium` or `Strong` â€” a median of 1, 5, 15 or 51 samples |
...
Smoothing here affects **drawing only**. Every measurement â€” insights, VE
calibration, the histogram, the scatter, the statistics and every export â€” reads
the channel as logged. See [User guide â–¸
Smoothing](User-guide#smoothing).
...
Full field reference: [Subaru SSM â–¸ Parameter file
reference](Subaru-SSM#parameter-file-reference).
...
| --- | --- | ---: |
| Colour scheme | **View â–¸ Theme**, or the box at the top right | Midnight |
| Units | **View â–¸ Units** | As reported |
...
| Histogram columns / rows | Histogram side panel | 16 Ã— 16 |
...
| Logging rate | **Tools â–¸ Logging rate** | 25 Hz |
| Record as soon as I connect | **Tools** | Off |
| Read the block in one request | **Tools** | Off |
| Data folder | **File â–¸ Data folder â–¸ Change the folderâ€¦** | `%USERPROFILE%\OpenLogViewer` |
...
- [Command line](Command-line) â€” overriding some of these for one run
