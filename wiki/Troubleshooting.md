...
| --- | --- | --- |
| "Windows protected your PC" on the installer | The installer is not code-signed | Expected. **More info â–¸ Run anyway** |
...
| Symptom | Likely cause | What to check |
| --- | --- | --- |
| "â€¦ is not a recognised datalog format" | No reader would accept the file | Open it in a text editor. Delimited text needs a header row followed by numeric data |
...
| A renamed file will not open | *(This should not happen)* | Nothing is assumed from the extension â€” content is examined. Report the file |

If a log from an ECU not listed in the [supported
formats](User-guide#supported-log-formats) does not open, the format is usually
easy to add â€” the delimited reader is one file.
...
| A trace is a flat line at its own mid-height | The channel barely moves, and steady channels are drawn as steady | **View â–¸ Draw steady channels as steady**, turn it off |
...
| Everything is squashed against the axis | One plotted channel reaches a huge value | Pin a scale on the offending channel, or `clamp(â€¦)` it if it is calculated |
| Two traces are the same colour | A pinned colour is in use | Pinned colours are not re-picked. Right-click â–¸ **Back to automatic** |
| A duplicate channel name appears twice | The firmware emits it twice in different units | Expected â€” MS3 emits `Fuel Consumption` in both GPH and l/hr. They are disambiguated by units |
| A calculated channel has no data | An input channel is not in this log | The sidebar reports which definition did not fit |
| A trace looks clean but the histogram disagrees with it | The channel is smoothed | Smoothing is drawing only; every measurement reads the channel as logged. The row says `smoothed Â· median of N` |
| A trace is smoother than the log should be | Smoothing was pinned on that channel in an earlier session | It persists by channel name. Right-click â–¸ **Back to automatic** |
...
| Symptom | Likely cause | What to check |
| --- | --- | --- |
| "The ECU reports â€¦ and no definition file on this machine matches it" | The `.ini` for that exact firmware build is missing | The message names the signature. Put the matching `.ini` in `%USERPROFILE%\OpenLogViewer\ECU definitions` |
...
| The device is not listed at all | It is Bluetooth LE, or a Wi-Fi dongle | BLE adapters show `(Bluetooth LE)`. Wi-Fi dongles appear in no list â€” see below |
...
| The session dies the moment it starts | **Read the block in one request** is on and the firmware cannot do it | **Tools â–¸ Read the block in one request**, turn it off |
...
| Symptom | Likely cause | What to check |
| --- | --- | --- |
| A Bluetooth adapter never becomes a COM port | It is Bluetooth LE, which has no serial port profile | It never will. Look for it in **Connect â–¾** with `(Bluetooth LE)` after the name |
| A Wi-Fi adapter appears nowhere | Correct â€” it is an access point, not a device | Join its Wi-Fi network, then **Connect â–¾ â–¸ Connect to a Wi-Fi OBD2 adapter** |
...
| "Nothing on COM*n* answered as an OBD2 adapter" | Wrong port, or it is a tuning cable | Try **Connect â–¾ â–¸ Connect as an OBD2 adapter** for a generic `USB-SERIAL CH340` |
...
| Symptom | Likely cause | What to check |
| --- | --- | --- |
| Connecting did not start a recording | **Record as soon as I connect** is off, which is the default | Press **â— Recordâ€¦**, or turn the option on under **Tools** |
...
| The recordings folder is not where expected | The data folder was changed | **File â–¸ Data folder â–¸ Open the folder** |
...
| The plot stopped following live data | You zoomed or panned | From then on you are reading history. **View â–¸ Reset zoom** goes back to watching |
...
| The suggested table looks nothing like the tune | The opened tune is not the one the log ran | **File â–¸ Use the tune stored in the log** |
| An axis was rejected | It is one of the rolled `â€¦doz` variants, which are not stored in order | Handled deliberately. Pick a different table |
...
| **Send to ECU** is greyed out | Nothing changed, or the tune came from a file | A tune opened from a file cannot be sent. Use **Tools â–¸ Restore a saved tune to the ECUâ€¦** |
...
| A value will not accept what you type | The firmware's declared range is tighter than the storage | The declared range is enforced. MS2Extra declares âˆ’10 Â° to 90 Â° for ignition where the encoding would take Â±3,276 Â° |
...
| Symptom | Likely cause | What to check |
| --- | --- | --- |
| The client reports `ConnectionRefused` | The server is not armed | This is the design working. **AI agent â–¸ Allow an AI agent to connect (MCP)** |
...
| The agent cannot clear fault codes or restore a tune | Neither is offered as a tool | Deliberate. Both are done by a person â€” see [mcp-server.md](AI-agent-access-MCP) |

Full detail: [AI agent access (MCP)](AI-agent-access-MCP).
...
| A pinned colour, scale or smoothing will not save | `channels.json` is at its 500-channel limit | Right-click a channel you no longer pin â–¸ **Back to automatic**, then try again |
...
| A device keeps appearing that is no longer here | It is in the remembered list | **Tools â–¸ Forget remembered ECUs**. Presets, filters and calculated channels are untouched |
