...
public class AgentServerTests : IDisposable
{
    ...
    private sealed class Bench : IAgentBridge
    {
        ...
        public List<(IReadOnlyList<ProposedSetting> Settings, IReadOnlyList<ProposedCell> Cells, string Note, bool ConfirmDangerous)> Applied { get; } = [];
        ...
        {
            Applied.Add((settings, cells, note, confirmDangerous));
            return NextApply;
        }
    ...
    public async Task ApplyingGoesThroughOnceArmedAndSaysWhatChanged()
    {
        ...
        (_, _, string note, _) = bench.Applied.Single();
